...
class MetadataWatcher:
    ...
    async def _fetch_and_stream(self, loc_cam: str, md_obj: dict[str, str]) -> None:
        ...
        try:
            ...
            if not data:
                logger.warning(
                    "MetadataWatcher: empty data from S3, clearing hash for retry",
                    loc_cam=loc_cam,
                    key=md_obj["key"],
                )
                self._fetched_hashes.pop(loc_cam, None)
                return

            if (
                loc_cam not in self._metadata_store
                or data != self._metadata_store[loc_cam]
            ):
            ...
            logger.exception(
                "MetadataWatcher: _fetch_and_stream failed, clearing hash for retry",
                loc_cam=loc_cam,
            )
            self._fetched_hashes.pop(loc_cam, None)
